...
print('GRANDE SELECTION HOMOGENE')
...
#Les deux lignes ci dessous : Taille echantillon : 1200
df.drop(df.index[600:sumf], inplace=True)
df.drop(df.index[1200:], inplace=True)
...
lrsgd = SGDClassifier(loss='hinge', penalty='l2',
                            alpha=1e-3, random_state=42,
                            max_iter=5, tol=None)
clf = lrsgd.fit(X_train_tfidf, train.target)
...
contentSDG = classification_report(train.target, pred_train_lrsgd)
print(contentSDG)
fichier = open("Classification_report_grande_SDG.txt", "a")
fichier.write(contentSDG)
fichier.close()
...
# Voir ACCURACY
pred_train_lrsgd = clf.predict(X_train_tfidf)
print('Logistic Regression with MultinomialNB Model training dataset accuracy: {0:0.4f}'. format(metrics.accuracy_score(train.target, pred_train_lrsgd)))

print('\nClassification report')
contentMulti = classification_report(train.target, pred_train_lrsgd)
print(contentMulti)
fichier = open("Classification_report_grande_MultinomialNB.txt", "a")
fichier.write(contentMulti)
fichier.close()
...
skplt.metrics.plot_confusion_matrix(train.target, pred_train_lrsgd,
                                    normalize=True,
                                    title="Grande selection MultinomialNB",
                                    cmap="Purples",
                                    ax=ax
                                    )
plt.show()
...
print(contentMPL)
fichier = open("Classification_report_grande_MPL.txt", "a")
...
skplt.metrics.plot_confusion_matrix(train.target, pred_train_lrsgd,
                                    normalize=True,
                                    title="Grande selection MPL 5 iterations",
                                    cmap="Purples",
                                    ax=ax
                                    )
plt.show()
...
clf = make_pipeline(StandardScaler(with_mean=False), SVC(gamma='auto')).fit(X_train_tfidf, train.target)
...
# Voir ACCURACY
pred_train_lrsgd = clf.predict(X_train_tfidf)
print('Logistic Regression with SVC Model training dataset accuracy: {0:0.4f}'. format(metrics.accuracy_score(train.target, pred_train_lrsgd)))
print('\nClassification report')
contentSVC = classification_report(train.target, pred_train_lrsgd)
print(contentSVC)
fichier = open("Classification_report_grande_SVC.txt", "a")
fichier.write(contentSVC)
fichier.close()
